...
os.chdir(my_path)
file=open("M1_macrophage_calibrated.json")
global model
model = json.load(file)
file.close()

df = pd.read_csv('a-computational-framework-to-build-and-calibrate-large-scale-boolean-models-main\\In_silico_simulations\\M1_targets_nodes.csv',sep=';', header=0)
ko=df['Target_nodes']
...
combi=[",".join(map(str, comb)) for comb in combinations(ko_sep_with_targets['Target_nodes'],2)]

def drug_combination(pair,co):
    args='BioCheckConsole.exe -engine VMCAI -model M1_macrophage_calibrated.json -prove stability_analysis.json'
    pair=pair.split(',')
    data=ko_sep_with_targets[ko_sep_with_targets['Target_nodes'].isin(pair)]
    ...
    df_stable.to_csv('a-computational-framework-to-build-and-calibrate-large-scale-boolean-models-main\\In_silico_simulations\\Two_by_Two_KO_simulation_results_M1\\Simulation'+'_'+str(co)+'.csv')
