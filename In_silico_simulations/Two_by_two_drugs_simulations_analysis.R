...
drug_combi<- read.csv("two_by_two_combination_KO_M2.csv")
...
for (i in files) {
  ...
  if ("apoptosis_survival_M2_macrophage_phenotype" %in% modif$stable_nodes_name)
  { res=stri_replace_all_regex(i,
                               pattern=c('Simulation_', '.csv'),
                               replacement=c('', ''),
                               vectorize=FALSE)
...
write.csv(drug_combi_interesting,"drug_combination_M1_osteoclastogenesis_inhibition.csv")
